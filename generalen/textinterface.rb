...
module TextInterface
  ...
  STANDARD_MAP = '~~~~~~~~~~~~_____~~~______________~~~~~~~~~~~~~~~~~~~__________~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
~~~~~~~~~~~/     \~/              \~~~~~~~~~~~~~~~~~/     /    \~~~~~___~~~~~~~~~~_________~~~~
      ...
      when :turn_change
        if params[:to_person] == @person
          ...
          if params[:from_person] == @person
            next_game = @person.games.values.detect{ |g| (g.game.started and not g.game.first_placement_done) or g.game.in_turn(@person) }
            if next_game
              @person.go_to_impl(next_game.game)
            end
          end
        end
      when :cards
